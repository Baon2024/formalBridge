...
var _APIFunctionsForBackend = require("./APIFunctionsForBackend.js");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
// Correct way to import Stripe in ES modules
...
app.use('/webhook', _bodyParser.default.raw({
...
app.post('/api/confirm-ticket-sold', async (req, res) => {
  ...
  const emailHtml = "\n      <html>\n        <body>\n          <h1>Your ticket has been bought!</h1>\n          <p>".concat(ticket.formalEventName, " has been bought!</p>\n        </body>\n      </html>\n    ");
  try {
    // Send the email using Resend API
    const response = await resend.emails.send({
      from: "Acme <onboarding@resend.dev>",
...
app.post("/account_link", async (req, res) => {
  try {
    ...
    const accountLink = await stripe.accountLinks.create({
      ...
      refresh_url: "http://localhost:3006/refresh/".concat(account),
  ...
  */
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: "http://localhost:3006/successPage/".concat(documentId) //rdirect to /successpage/${documentId}
...
app.post('/webhook', _express.default.raw({
  type: 'application/json'
}), (req, res) => {
  let event = req.body;
  ...
  switch (event.type) {
    case 'payment_intent.succeeded':
      ...
      console.log("PaymentIntent for ".concat(paymentIntent.amount, " was successful!"));
    ...
    case 'checkout.session.completed':
      ...
      if (globalTicket && globalUser) {
        (0, _APIFunctionsForBackend.setTicketBought)(globalTicket, jwtToken);
        (0, _APIFunctionsForBackend.updateBuyerUser)(globalTicket, globalUser, jwtToken);
      ...
      if (metadata) {
        ...
        ticketIds.map(ticketId => {
          (0, _APIFunctionsForBackend.setTicketBoughtMultiple)(ticketId, jwtToken);
          (0, _APIFunctionsForBackend.updateBuyerUserMultiple)(ticketId, buyerUserId, jwtToken);
  ...
  */
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: "http://localhost:3006/successPage/".concat(documentId, "?session_id={CHECKOUT_SESSION_ID}")
...
app.post('/create-checkout-session-destination-embedded', async (req, res) => {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      ui_mode: 'embedded',
      return_url: "http://localhost:3006/destinationPage/".concat(documentId, "?session_id={CHECKOUT_SESSION_ID}")
...
app.post('/verify-payment', async (req, res) => {
  const {
    sessionId
  } = req.body; // The session ID passed from frontend
...
app.post('/create-checkout-session-multiple', async (req, res) => {
  ...
  try {
    ...
    const ticketsBySeller = cart.reduce((acc, ticket) => {
      var _ticket$sellerUser;
      const sellerAccountId = (_ticket$sellerUser = ticket.sellerUser) === null || _ticket$sellerUser === void 0 ? void 0 : _ticket$sellerUser.connectedAccountId;
      if (!sellerAccountId) {
        throw new Error("Ticket ".concat(ticket.formalEventName, " is missing a seller connectedAccountId."));
      }
    ...
    const paymentIntents = await Promise.all(Object.entries(ticketsBySeller).map(async _ref => {
      let [sellerAccountId, tickets] = _ref;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: "http://localhost:3006/successPage/".concat(totalCartIds, "?session_id={CHECKOUT_SESSION_ID}"),
      /*cancel_url: `${YOUR_DOMAIN}/cancel`,*/
      metadata: {
        ticketIds: "".concat(totalCartIds),
        // Comma-separated ticket IDs
        buyerUserId: "".concat(user.user.id) // Buyer user ID
      }
    });
...
app.post('/create-checkout-session-multiple-embedded', async (req, res) => {
  ...
  const session = await stripe.checkout.sessions.create({
    payment_method_types: ['card'],
    line_items: [],
    // We'll populate this with seller line items
    mode: 'payment',
    success_url: "http://localhost:3006/successPage/".concat(totalCartIds, "?session_id={CHECKOUT_SESSION_ID}"),
  ...
  // Group tickets by seller
  const ticketsBySeller = cart.reduce((acc, ticket) => {
    var _ticket$sellerUser2;
    const sellerAccountId = (_ticket$sellerUser2 = ticket.sellerUser) === null || _ticket$sellerUser2 === void 0 ? void 0 : _ticket$sellerUser2.connectedAccountId;
    if (!sellerAccountId) {
      throw new Error("Ticket ".concat(ticket.formalEventName, " is missing a seller connectedAccountId."));
    }
    if (!acc[sellerAccountId]) acc[sellerAccountId] = [];
  ...
  let totalAmount = 0;
  Object.entries(ticketsBySeller).forEach(_ref2 => {
    var _tickets$0$sellerUser;
    let [sellerAccountId, tickets] = _ref2;
    ...
    session.line_items.push({
      price_data: {
        currency: 'gbp',
        product_data: {
          name: "Tickets from ".concat((_tickets$0$sellerUser = tickets[0].sellerUser) === null || _tickets$0$sellerUser === void 0 ? void 0 : _tickets$0$sellerUser.name),
...
app.use('/getTickets', async (req, res, next) => {
  try {
    ...
    const fetchedFilteredTickets = await (0, _APIFunctionsForBackend.fetchTicketsData)();
